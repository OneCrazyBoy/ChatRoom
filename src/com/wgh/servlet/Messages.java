...
public class Messages extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        response.setContentType("text/html;charset=UTF-8");//设置相应页面的编码格式
		String action = request.getParameter("action"); //通过getParameter获取到上一个页面传过来的action
	...
	public void getMessages(HttpServletRequest request,HttpServletResponse response) {
        response.setContentType("text/html;charset=UTF-8");  //设置页面内容类型
        
        try {
            request.getRequestDispatcher("content.jsp").forward(request, response);//这就是站内转发，要好好理解！！！
        } 
	...
	public void loginRoom(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
		response.setContentType("text/html;charset=UTF-8"); //设置内容类型为UTF-8
		...
		Vector vector=user.getList();   //实例化一个vector对象
		boolean flag=true;		        //标记是否登录的变量
		//判断用户是否登录
		System.out.println("vector的size："+vector.size());
		if(vector!=null&&vector.size()>0){
			for(int i=0;i<vector.size();i++){    //遍历vector
